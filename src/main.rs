...
struct Player {
    ...
    remaining_actions: SmallVec<[Action; 4]>,
    // Cards in their hand
    hand: Cards,
...
impl GameState {
    ...
    fn new(rng: Box<dyn rand::RngCore>) -> GameState {
        let mut gs = GameState {
            deck: vec![],
            places: [Place::new(2), Place::new(2), Place::new(2), Place::new(3), Place::new(3), Place::new(4), Place::new(5)],
    ...
    fn reset(&mut self) {
        ...
        self.players[0].remaining_actions = smallvec![Action::Secret, Action::Discard, Action::Gift, Action::Competition];
        self.players[1].remaining_actions = smallvec![Action::Secret, Action::Discard, Action::Gift, Action::Competition];
    }
    ...
    fn update_control_and_score(&mut self) -> Option<PlayerId> {
        ...
        let (score, places_controlled) = self.places.iter().fold(([0,0], [0, 0]), |(mut score, mut places), place| {
            match place.control {
                Some(pid) => {
        ...
        match score.iter().enumerate().max_by_key(|(_, &s)| s) {
            Some((player_index, &m)) if m >= 11 => {
                return Some(PlayerId::of_index(player_index))
            },
            Some(_) | None => { }
        }
    ...
    fn play<B: Behavior>(mut self, b: &mut B) -> PlayerId {
        ...
        loop {
            ...
            match action {
                ...
                Action::Competition => {
                    ...
                    for (hand, player) in b.assign(current_player.other(), [first_set, second_set]).iter()
                    {
