...
pub struct CardsWidget<'a>(
    pub &'a crate::mechanics::Cards,
    pub &'a crate::mechanics::GameState,
    pub bool,
);
...
impl<'a> Widget for CardsWidget<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        for (card_idx, card) in self.0.iter().enumerate() {
            ...
            let card_text = draw_card(
                if self.2 {
                    ' '
                } else {
                    char_of_digit(self.1.places[card.0 as usize].face_value)
                },
                1,
            );
            for (count, line) in card_text.lines().enumerate() {
                buf.set_string(
                    area.left() + 3 * (card_idx as u16),
                    area.top() + (count as u16),
                    line,
    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let main_square = format!(
            "\
             ...
             │    {}│\n\
             ╘═════╛",
            self.0.face_value, self.0.face_value
        );
        let bottom_square = draw_card(char_of_digit(self.0.face_value), self.0.scores[1] as usize);
            ...
            .enumerate()
        {
            ...
            buf.set_string(
                ...
                area.top() + count,
                line,
                Style::default().fg(color_from_place(self.1)),
            );
        }
    }
}
